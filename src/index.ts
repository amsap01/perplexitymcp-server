...
export async function startServer() {
  ...
  const isClaudeOrInspector = false
  ...
  if (isClaudeOrInspector) {
    ...
    app.post("/messages", express.json(), async (req: Request, res: Response) => {
      try {
        ...
        const rpc = req.body;
        if (rpc && rpc.jsonrpc === "2.0" && rpc.method === "tools/call" && rpc.params && rpc.params.name === "search_perplexity") {
          // Map arguments and set default model to 'sonar-pro' if not provided
          const args = { model: 'sonar-pro', ...rpc.params.arguments };
          console.log("ARGS SENT TO perplexityChat:", JSON.stringify(args, null, 2));
          const result = await perplexityChat(args);
          res.json({
            jsonrpc: "2.0",
            id: rpc.id,
          ...
          // Fallback: direct call (stateless, legacy)
          const args = { model: 'sonar-pro', ...req.body };
          const result = await perplexityChat(args);
          res.json({
            content: result.content.map(item => ({
              type: "text",
  ...
  console.log("HTTP server started and running...");
  setInterval(() => {}, 1 << 30); // Keeps the event loop alive
    });
